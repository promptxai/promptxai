...
def ideas(keywords, count=10):
    """Generates a list of ideas by combinining keywords.

    Args:
        keywords: Space separated keywords to combine into ideas.
    ...
    Returns:
        ...
        response_list: Completion as a Python list.
    """
    prompt = 'As an expert, brainstorm a list of ' + str(count)
    prompt += ' ideas combining the keywords: ' + keywords + '.'
...
def elaborate(text, words=50):
    """Takes a text and describes it in given number of words with grade N level English.

    Args:
        text: Text to be described.
    ...
    """
    prompt = 'Like an expert, using narrative style, elaborate the text: ' + text + '.'
    completion = core.openai_completion(prompt, 
        tokens=core.words_to_tokens(words), 
        temperature=0.9).choices[0].text.strip()
    return strip_trailing_sentence(completion)

# check grammar of a text and return suggestions
def grammar(text):
    """Takes a text and returns grammar suggestions.

    Args:
        text: Text to be checked for grammar.
    
    Return:
        completion: Completion as paragraph(s) of free text.
    """
    prompt = 'If correct English grammar is used, respond by saying so, otherwise respond with corrected text:\n' + text
    completion = core.openai_completion(prompt, 
        tokens=core.words_to_tokens(len(text.split())) + 50, 
        temperature=0).choices[0].text.strip()
    return completion

# extract a table from a text
def tabulate(text):
    """Extracts a table from a text.

    Args:
        text: Text to extract table from.
    
    Return:
        completion: Completion as a markdown table.
    """
    prompt = 'Extract a markdown table based on related concepts and data in the text:\n' + text
    completion = core.openai_completion(prompt, 
        tokens=core.words_to_tokens(len(text.split())), 
        temperature=0).choices[0].text.strip()
    return strip_trailing_sentence(completion)

def numbered_list(text, count=10):
    """Extracts a list from a text.

    Args:
        text: Text to extract table from.
    
    Return:
        completion: Completion as a numbered list.
        completion_list: Completion as a Python list.
    """
    prompt = 'Suggest ' + str(count) + ' concepts based on the text:\n' + text
    return completion_list(prompt, tokens=count*20)

def titles(text, count=10):
    """Generate list of blog post titles based on text.
    ...
    prompt += ' catchy, unique, and SEO friendly blog post titles based on the text: ' + text
    return completion_list(prompt, tokens=count*20)

def tags(text, count=10):
    """Generates count number of tags based on a given text.
    ...
    Returns:
        ...
        response_list: Completion as a Python list.
    """
    prompt = 'Suggest a numbered list, ordered by popularity, of ' + str(count) 
    prompt += ' tags based on the text:\n' + text
    return completion_list(prompt, tokens=count*20)
...
def summarize(text, words=20):
    """Summarizes a text to N words and writes the completion in grade N level English.

    Args:
        text: Free text paragraph(s) to be summarized.
        words: Number of words/tokens to be used by completion.
        grade: Completion will use this grade level English.
    ...
    """
    prompt = 'Summarize in ' + str(words) + ' words: ' + text + '.'
...
def generate_html(post):
    ...
    if post.get_auto():
        ...
        post_html += '''This blog post was 100% AI generated using PromptxAI API. 
...
def auto_generate(keywords, folder='', verbose = False):
    ...
    # Ideate
    response, response_list = ideas(keywords)
    ...
    html_path, markdown_path = publish(
        post_html=post_html, 
        folder=folder, 
...
def list_files(folder):
    ...
    Args:
        folder: Folder to list files in.
    Returns:
        file_list: List of files in the folder.
    """
    file_list = []
    for root, dirs, files in core.os.walk(folder):
        for file in files:
